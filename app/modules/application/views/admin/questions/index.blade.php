                            ...
                            <td class="text-center" style="white-space:nowrap;">
                                @if(Allow::action('application','edit'))
                                    <a class="btn btn-success margin-right-5"
                                       href="{{ URL::route('questions.edit', $question->id) }}" title="Изменить">
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                @endif
                                @if(Allow::action('application','delete'))
                                    {{ Form::open(array('route' => array('questions.destroy', $question->id),'method'=>'delete','style'=>'display:inline-block')) }}
                                    <button type="button" class="btn btn-danger remove-question"><i
